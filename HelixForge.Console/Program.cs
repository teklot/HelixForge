...
/// <summary>
/// Golden Path Demo: UAV stabilization system running entirely in simulation.
/// Demonstrates device abstraction, simulation, telemetry, and closed-loop control.
/// </summary>
class Program
{
    static void Main(string[] args)
    {
        ...
        double targetAngle = 0.0; // Target: level flight

        System.Console.WriteLine("Starting simulation...");
        System.Console.WriteLine($"Time step: {simConfig.TimeStep.TotalMilliseconds}ms");
        System.Console.WriteLine($"Target angle: {targetAngle} rad");
        System.Console.WriteLine();

        // Run for 5 seconds (500 steps at 100Hz)
        engine.Run(TimeSpan.FromSeconds(5), currentTime =>
        {
            ...
            integral += error * simConfig.TimeStep.TotalSeconds;
            double derivative = (error - previousError) / simConfig.TimeStep.TotalSeconds;
            ...
            if (currentTime.TotalMilliseconds % 500 < simConfig.TimeStep.TotalMilliseconds)
            {
                System.Console.WriteLine(
